...
function parseCliArgs(argv) {
  const args = { input: "data.json", output: "to_trinh.docx" };
...
async function generateToTrinh(jsonPath, outputPath) {
  const data = readJsonFile(jsonPath);
  const required = ["co_quan_chu_quan", "co_quan_ban_hanh", "kinh_gui", "noi_dung", "chuc_vu_ky", "nguoi_ky"];
  const missing = required.filter(k => !data[k]);
  if (missing.length) throw new Error(`Missing: ${missing.join(", ")}`);

  const diaDanh = data.dia_danh || "Hà Nội";
  const soKyHieu = data.so_ky_hieu || "Số: ..../TTr-...";
  ...
  // Tên loại: TỜ TRÌNH
  children.push(paraText("TỜ TRÌNH", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 200, after: 120 } }));
  ...
  children.push(paraText(`Kính gửi: ${kinhGui}`, { alignment: AlignmentType.LEFT, noIndent: true, size: 28, italics: true, spacing: { before: 120, after: 200 } }));
...
generateToTrinh(args.input, args.output).catch(err => { console.error("Error:", err.message); process.exit(1); });
