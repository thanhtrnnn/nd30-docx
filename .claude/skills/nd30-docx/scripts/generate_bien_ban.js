...
function parseCliArgs(argv) {
  const args = { input: "data.json", output: "bien_ban.docx" };
...
async function generateBienBan(jsonPath, outputPath) {
  const data = readJsonFile(jsonPath);
  const required = ["co_quan_chu_quan", "co_quan_ban_hanh", "noi_dung", "chuc_vu_ky", "nguoi_ky"];
  const missing = required.filter(k => !data[k]);
  if (missing.length) throw new Error(`Missing: ${missing.join(", ")}`);

  const diaDanh = data.dia_danh || "Hà Nội";
  const soKyHieu = data.so_ky_hieu || "Số: ..../BB-...";
  ...
  // Tên loại: BIÊN BẢN
  children.push(paraText("BIÊN BẢN", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 200, after: 120 } }));

  // Trích yếu
  if (data.trich_yeu) {
    children.push(paraText(data.trich_yeu, { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 60, after: 200 } }));
  }

  // Thông tin biên bản: thời gian, địa điểm, thành phần
  if (data.thoi_gian) {
    children.push(paraText(`Thời gian: ${data.thoi_gian}`, { alignment: AlignmentType.JUSTIFIED, size: 28 }));
  }
  if (data.dia_diem) {
    children.push(paraText(`Địa điểm: ${data.dia_diem}`, { alignment: AlignmentType.JUSTIFIED, size: 28 }));
  }
  if (data.thanh_phan) {
    children.push(paraText(`Thành phần tham gia: ${data.thanh_phan}`, { alignment: AlignmentType.JUSTIFIED, size: 28 }));
  }
  if (data.chu_tri) {
    children.push(paraText(`Chủ trì: ${data.chu_tri}`, { alignment: AlignmentType.JUSTIFIED, size: 28 }));
  }
  if (data.thu_ky) {
    children.push(paraText(`Thư ký: ${data.thu_ky}`, { alignment: AlignmentType.JUSTIFIED, size: 28 }));
  }
  ...
  if (Array.isArray(data.noi_dung)) {
    data.noi_dung.forEach(block => {
      if (typeof block === "string") {
        ...
        const pChildren = [];
        if (block.tieu_de) pChildren.push(new TextRun({ text: block.tieu_de + " ", font: "Times New Roman", size: 28, bold: true }));
        if (block.noi_dung) pChildren.push(new TextRun({ text: ensureEndingPunctuation(block.noi_dung), font: "Times New Roman", size: 28 }));
  ...
  // Dual signature: người ghi + chủ trì
  ...
  // Left: người ghi biên bản
  const ghiBBChildren = [
    paraText("NGƯỜI GHI BIÊN BẢN", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 0, after: 60 } }),
    paraText("(Ký, ghi rõ họ tên)", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, italics: true, spacing: { before: 40, after: 500 } }),
    paraText(data.nguoi_ghi || "", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 0, after: 120 } }),
  ];

  // Right: chủ trì
  const chuTriChildren = [
    paraText(data.chuc_vu_chu_tri || "CHỦ TRÌ", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 0, after: 60 } }),
    paraText("(Ký, ghi rõ họ tên)", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, italics: true, spacing: { before: 40, after: 500 } }),
    paraText(data.nguoi_chu_tri || "", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 0, after: 120 } }),
  ];

  // Footer: noi nhan + dual signature
  children.push(new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
    rows: [new TableRow({ children: [
      new TableCell({ width: { size: 55, type: WidthType.PERCENTAGE }, borders: noBorders(), children: [
        ...noiNhanChildren,
        new Paragraph({ spacing: { before: 200, after: 0 } }),
        ...ghiBBChildren,
      ]}),
      new TableCell({ width: { size: 45, type: WidthType.PERCENTAGE }, borders: noBorders(), children: chuTriChildren }),
    ]})],
  }));
...
generateBienBan(args.input, args.output).catch(err => { console.error("Error:", err.message); process.exit(1); });
