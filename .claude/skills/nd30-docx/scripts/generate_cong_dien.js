// Công điện: Nhóm B - giống công văn nhưng có tên loại "CÔNG ĐIỆN"
// Sử dụng chung logic với công văn, thêm trường hợp đặt biệt
const fs = require("fs");
const {
...
function parseCliArgs(argv) {
  const args = { input: "data.json", output: "cong_dien.docx" };
...
async function generateCongDien(jsonPath, outputPath) {
  const data = readJsonFile(jsonPath);
  const required = ["co_quan_chu_quan", "co_quan_ban_hanh", "trich_yeu", "kinh_gui", "noi_dung", "chuc_vu_ky", "nguoi_ky"];
  const missing = required.filter(k => !data[k]);
  if (missing.length) throw new Error(`Missing: ${missing.join(", ")}`);

  const diaDanh = data.dia_danh || "Hà Nội";
  const soKyHieu = data.so_ky_hieu || "Số: ..../CD-...";
  const ngayThang = formatNgayThang(data.ngay_thang);

  const children = [];

  // Header (same as cong van)
    ...
    rows: [
      new TableRow({ children: [
        new TableCell({ width: { size: 3500, type: WidthType.DXA }, borders: noBorders(), children: [
          ...
          paraText(`V/v ${data.trich_yeu.replace(/^Về việc\s+/i, "")}`, { alignment: AlignmentType.CENTER, noIndent: true, size: 24, spacing: { before: 60, after: 0 } }),
        ]}),
  ...
  // CÔNG ĐIỆN
  children.push(paraText("CÔNG ĐIỆN", { alignment: AlignmentType.CENTER, noIndent: true, size: 28, bold: true, spacing: { before: 200, after: 120 } }));

  // Kính gửi
  const kinhGui = Array.isArray(data.kinh_gui) ? data.kinh_gui.join("; ") : data.kinh_gui;
  children.push(paraText(`Kính gửi: ${kinhGui}`, { alignment: AlignmentType.LEFT, noIndent: true, size: 28, spacing: { before: 120, after: 200 } }));
...
generateCongDien(args.input, args.output).catch(err => { console.error("Error:", err.message); process.exit(1); });
